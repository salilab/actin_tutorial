...
class AnalysisTrajectories(object):
    ...
    def set_analyze_SAXS_restraint(self):
        self.restraint_handles.append('SAXSRestraint_Score')    
        self.restraint_names['SAXSRestraint_Score']='SAXS'
        self.SAXS_restraint = True
    ...
    def read_traj_info(self, out_dirs_sel):
        ...
        for out in out_dirs_sel:
            ...
            #print([v for v in S_dist.columns.values if 'distance' in v])
            #print(S_dist.columns.values)
            #exit()
            # Also add nuisances parameter
            #sel_entries += [v for v in S_tot_scores.columns.values if 'Psi' in v and 'sum' not in v]
            # Get rid of psi
            sel_entries = [i for i in sel_entries if not ('psi' in i)]
            ...
            if out in self.dir_halfA:
                ...
                S_dist_clust = S_dist.assign(half = pd.Series(['A']*len(S_dist),index=S_dist.index).values)
            elif out in self.dir_halfB:
                S_tot_scores = S_tot_scores.assign(half = pd.Series(['B']*len(S_tot_scores),
                                                                    index=S_tot_scores.index).values)
                S_dist_clust = S_dist.assign(half = pd.Series(['B']*len(S_dist),index=S_dist.index).values)
            else:
                S_tot_scores = S_tot_scores.assign(half = pd.Series([0]*len(S_tot_scores),
                                                                    index=S_tot_scores.index).values)
                S_dist_clust = S_dist.assign(half = pd.Series([0]*len(S_dist),index=S_dist.index).values)

            # Collect distances and nuisances information
            if self.cluster_distances:
                S = S_dist_clust #S_tot_scores[ts_max:]
                #print("S:",S.keys())
                #print("S_dist_clust",S_dist_clust.keys())
                #S.update(S_dist_clust[:])
                print(S.keys())
                self.S_all[out] = S
                #exit()
            else:
    ...
    def write_hdbscan_clustering(self, S_comb):
        ...
        for cl in clusters:
            ...
            if len(HA['half'].values) >= 10 and len(HB['half'].values) >= 10:
                ...
                # Select n model from
                if int(n) > 20000  or len(HH_cluster) > 20000:
                    HH_sel = HH_cluster.sample(n=19999)
    ...
    def extract_models_single_rmf(self, gsms_info, out_rmf, model_dir="", random=1.0):
        import IMP
        import IMP.rmf
        import RMF

        if random < 0.0:
            raise Exception("Random sample must be > 0 and < 1.0.", random, "was inputted")
        else:
            tot_rows = len(gsms_info.index)
            rows = np.random.choice(gsms_info.index.values, int(len(gsms_info.index)*random))
            new_df = gsms_info.ix[rows]
            gsms_info = new_df

        print("Extracting", len(rows), "of", tot_rows,"models")

        scores = []
        init = False
        i=0
        for (row_id, row) in gsms_info.iterrows():
            t = row.traj
            fr = int(row.MC_frame)
            fr_rmf = int(row.rmf_frame_index)
            flie = row.rmf3_file
            rmf_file = model_dir+flie

            if not init:
                m = IMP.Model()
                f = RMF.open_rmf_file_read_only(rmf_file)
                [h0] = IMP.rmf.create_hierarchies(f, m)
                fh_A = RMF.create_rmf_file(out_rmf)
                IMP.rmf.add_hierarchy(fh_A, h0)
                IMP.rmf.save_frame(fh_A, str(0))
                del f
                init = True
                ...
                IMP.rmf.save_frame(fh_A, str(i))
                del f

            if i%10 == 0:
                print("Writing frame:", i, "of", len(rows))

            # Collect scores
            scores.append(row.Total_Score)
        # Write scores to file
        np.savetxt('./scores_A.txt', np.array(scores))

    def do_extract_models(self, gsms_info, filename, gsms_dir, model_dir=""):
        ...
        # Setup a list of processes that we want to run
        processes = [mp.Process(target=self.extract_models, args=(df_array[x], filename, gsms_dir, model_dir)) for x in range(self.nproc)]
    ...
    def get_models_to_extract(self, file, number=None):
        # Get models to extract from file
        DD  = pd.read_csv(file)
        if number:
            return DD.sample(n=number)
    ...
    def extract_models(self, gsms_info, filename, gsms_dir, model_dir):
        '''
        ...
        for row in gsms_info.itertuples():
            ...
            #print("    ",model_dir, id, traj_in, model_dir+filename+'_'+str(id)+'_'+str(fr)+'.rmf3') 
            #print(file_out)
            #exit()
            os.system('rmf_slice -q '+model_dir+traj_in+ ' '+file_out+' --frame '+str(fr_rmf) )
    ...
    def boxplot_XLs_distances(self, cluster = 0, type_XLs = None, cutoff = 30.0):
        ...
        #print("BP,", self.ambiguous_XLs_restraint)
        if self.ambiguous_XLs_restraint == True:
