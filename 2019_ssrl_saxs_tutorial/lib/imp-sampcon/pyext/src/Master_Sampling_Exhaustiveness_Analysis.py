...
#sys.path.append("../lib/pyRMSD-4.2.1")
...
parser.add_argument('--rmfA', '-ra', dest="rmf_A", help='name of the file having the good-scoring scores for sample A')
parser.add_argument('--rmfB', '-rb', dest="rmf_B",help='name of the file having the good-scoring scores for sample B')
...
with open(args.scoreA, 'r') as f:
    for line in f:
        score_A.append(float(line.strip("\n")))

with open(args.scoreB, 'r') as f:
    for line in f:
...
t1 = time.time()
print("Score-based convergence estimates completed")
...
if args.extension == "pdb":
    ...
    args.extension = "rmf3"
    ps_names, masses, radii, conforms, models_name, n_models = get_rmfs_coordinates(args.path, args.rmf_A, args.rmf_B, args.subunit)

t2 = time.time()

print("Took", t2-t1, "to collect all model coordinates")
...
try:
    ...
    distmat = np.load("Distances_Matrix.data.npy")

distmat_full = sp.spatial.distance.squareform(distmat)

print("Size of RMSD matrix (unpacked, N x N):",distmat_full.shape)
print("Distance matrix calculation took", time.time()-t2)

# Get model lists

total_num_models=n_models[0]+n_models[1]

print("Size of Sample A:",n_models[0]," ; Size of Sample B: ",n_models[1],"; Total", total_num_models)
...
if not args.skip_sampling_precision:
    ...
    print("Clustering at thresholds:",cutoffs_list[0], "to", cutoffs_list[-1], "with step size", gridSize)
    print("   ",len(cutoffs_list), "total steps")
    
    t1 = time.time()

    # Do clustering at each cutoff
    pvals, cvs, percents = get_clusters(cutoffs_list, distmat_full, total_num_models, n_models[0], n_models[1], args.sysname)
...
ctable,retained_clusters=get_contingency_table(len(cluster_centers),cluster_members,n_models[0], n_models[1])
...
for rows in range(len(ctable)):
    print(str(rows)+" "+str(ctable[rows][0])+" "+str(ctable[rows][1]), file=fcp)
...
for i in range(len(retained_clusters)):
    ...
    conform_0 = conforms[cluster_members[clus][0]]

    # Add the cluster center model RMF to the cluster directory
    cluster_center_index = cluster_members[clus][0]
    cluster_center_model_id = cluster_center_index # index to Identities file.
    ...
    os.system('rmf_pdb -f 0 ./cluster.'+str(i)+'/cluster_center_model.rmf ./cluster.'+str(i)+'/cluster_center_model.pdb')
    # for each model in the cluster
    print("Building model for cluster", clus, "with", len(cluster_members[clus]),"members")
    j=0
    t1 = time.time()

    # Create a model with just the cluster_member coordinates
    model = IMP.Model()
    ps = []
    ...
    for mem in cluster_members[clus]:
        if j%100 == 0:
            print("Loading model", j+1, "   :::   100 models took", time.time()-t1, "seconds")
            t1 = time.time()
        model_index=mem
        j+=1
        tt = time.time() 
        # get superposition of each model to cluster center and the RMSD between the two
        rmsd, superposed_ps = get_particles_from_superposed(conforms[model_index], conform_0, args.align, ps)
        model.update()
        ...
        print(str(model_index),file=both_file)

        if model_index < n_models[0]:
            gmd1.add_subunits_density(superposed_ps) # density map for sample A
            print(str(model_index), file=sampleA_file)
        else:
            gmd2.add_subunits_density(superposed_ps) # density map for sample B
            print(str(model_index), file=sampleB_file)
    cluster_precision /= float(len(cluster_members[clus])-1.0)
    
    print("Cluster precision (average distance to cluster centroid) of cluster "+str(i)+" is %.3fA"%(cluster_precision),  file=fpc)
