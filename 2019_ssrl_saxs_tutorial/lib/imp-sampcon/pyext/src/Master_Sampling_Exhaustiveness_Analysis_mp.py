...
# Do this in multiprocessor mode
manager = mp.Manager()
ps_names = manager.dict()
masses = manager.dict()
radii = manager.dict()
conforms = manager.dict()
models_name = manager.dict()

output = mp.Queue()

processes = [mp.Process(get_rmfs_coordinates,
                                args=((out_dirs_dict[x],))) for x in range(nproc)]
